...
WITH events AS (
    ...
    SELECT
        ...
        identity_metadata.run_as as dbx_user,
...
-- Get most recent Genie space display name per space
space_names AS (
    SELECT
        workspace_id,
        request_params.space_id AS space_id,
        request_params.display_name AS space_name
    FROM genie_usage
    WHERE action_name = 'updateSpace'
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY workspace_id, request_params.space_id
        ORDER BY event_time DESC
    ) = 1
)
SELECT
    -- Union of Genie Events
    e.*,
    -- Space Name
    s.space_name,
    -- User Group information
    ug.groups       AS dbx_user_groups,
    ug.group_count  AS dbx_user_group_count
FROM
    events e
LEFT JOIN
    space_names s
ON
    e.workspace_id = s.workspace_id
AND
    e.space_id = s.space_id
LEFT JOIN
    user_groups ug
ON
    e.workspace_id = ug.workspace_id
AND
    e.dbx_user = ug.member_email
ORDER BY
    event_time_local_tz
;
